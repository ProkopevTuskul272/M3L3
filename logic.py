...
class DB_Manager:
    ...
    def default_insert(self):
        ...
        data = [ (_,) for _ in (['Python', 'SQL', 'API'])]
        self.__executemany(sql, data)
        sql = 'INSERT OR IGNORE INTO status (status_name) values(?)'
        data = [ (_,) for _ in (['На этапе проектирования', 'В процессе разработки', 'Разработан. Готов к использованию.', 'Обновлен', 'Завершен. Не поддерживается'])]
        self.__executemany(sql, data)


    def insert_project(self, data):
        sql = "INSERT INTO projects (project) values(?)" # Запиши сюда правильный SQL запрос
        self.__executemany(sql, data)
    ...
    def get_projects(self, user_id):
        sql = "SELECT user_id FROM projects" # Запиши сюда правильный SQL запрос
...
"""
    ...
    def update_projects(self, param, data):
        sql = "UPDATE projects" # Запиши сюда правильный SQL запрос
        self.__executemany(sql, [data, param]) 


    def delete_project(self, user_id, project_id):
        sql = "DELETE FROM projects WHERE user_id, project_id" # Запиши сюда правильный SQL запрос
        self.__executemany(sql, [(user_id, project_id)])
    
    def delete_skill(self, project_id, skill_id):
        sql = "DELETE FROM skills WHERE skill_id, skill_name" # Запиши сюда правильный SQL запрос
